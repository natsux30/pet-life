      ...
      <div style={{ background: '#f5f5f5', minHeight: '100vh', padding: '20px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{
            ...
            marginBottom: '20px', boxShadow: '0 3px 15px rgba(0,0,0,0.08)'
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', flexWrap: 'wrap', gap: '15px' }}>
              <div>
                ...
                <h1 style={{ margin: '0 0 5px 0', color: '#333', fontSize: '1.5rem' }}>
              ...
              <div style={{
                background: getStatusColor(pedidoSelecionado.rastreio.status) + '20',
                color: getStatusColor(pedidoSelecionado.rastreio.status),
                padding: '10px 20px', borderRadius: '25px',
                fontWeight: 'bold', fontSize: '0.9rem'
              }}>
                {getStatusIcon(pedidoSelecionado.rastreio.status)} {pedidoSelecionado.rastreio.status}
              </div>
            </div>
            ...
            <div style={{
              ...
              <div>
                <span style={{ color: '#999', fontSize: '0.8rem' }}>Código de Rastreio</span>
                <p style={{ margin: '3px 0 0 0', fontWeight: 'bold', color: '#333', fontSize: '1.1rem', letterSpacing: '1px' }}>
          ...
          <div style={{
            ...
            <h2 style={{ marginTop: 0, color: '#333', fontSize: '1.2rem', marginBottom: '25px' }}>
              📍 Linha do Tempo
            </h2>

            <div style={{ position: 'relative' }}>
              {pedidoSelecionado.rastreio.historico.map(function(evento, index) {
                const isLast = index === pedidoSelecionado.rastreio.historico.length - 1;
                ...
                return (
                  <div key={index} style={{
                    ...
                    {/* Conteúdo */}
                    <div style={{
                      flex: 1, background: isFirst ? '#e8f5e9' : '#f8f9fa',
                      padding: '15px', borderRadius: '10px'
                    }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', flexWrap: 'wrap', gap: '5px' }}>
                        <strong style={{ color: '#333' }}>{evento.status}</strong>
          ...
          <div style={{
            background: 'white', borderRadius: '15px', padding: '25px',
            marginTop: '20px', boxShadow: '0 3px 15px rgba(0,0,0,0.08)'
          }}>
            <h2 style={{ marginTop: 0, color: '#333', fontSize: '1.2rem', marginBottom: '20px' }}>
              🛍️ Itens do Pedido
            </h2>
            ...
            <div style={{ borderTop: '2px solid #f0f0f0', marginTop: '15px', paddingTop: '15px' }}>
              ...
              {pedidoSelecionado.desconto > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px', color: '#2e7d32', fontSize: '0.9rem' }}>
                  <span>Desconto</span>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px', color: '#666', fontSize: '0.9rem' }}>
                <span>Frete</span>
                <span>{pedidoSelecionado.frete === 0 ? 'Grátis' : `R$ ${pedidoSelecionado.frete.toFixed(2)}`}</span>
              </div>
              <div style={{
  ...
  return (
    <div style={{ background: '#f5f5f5', minHeight: '100vh', padding: '20px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <h1 style={{ color: '#333', marginBottom: '20px' }}>📦 Rastrear Pedido</h1>
        ...
        <div style={{
          ...
          <p style={{ color: '#666', marginTop: 0 }}>Digite o número do pedido para rastrear:</p>
          <div style={{ display: 'flex', gap: '10px' }}>
            <input
              type="text"
              placeholder="Número do pedido (ex: 123456)"
              value={codigoBusca}
              onChange={function(e) { setCodigoBusca(e.target.value); }}
          ...
          {erroBusca && (
            <p style={{ color: '#d63031', fontSize: '0.9rem', marginTop: '10px' }}>{erroBusca}</p>
          )}
        </div>
        ...
        {pedidoEncontrado && (
          <div style={{
            background: 'white', borderRadius: '15px', padding: '25px',
            boxShadow: '0 3px 15px rgba(0,0,0,0.08)', cursor: 'pointer'
          }}
          onClick={function() { selecionarPedido(pedidoEncontrado); }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
              <div>
                <h3 style={{ margin: '0 0 5px 0', color: '#333' }}>Pedido #{pedidoEncontrado.numero}</h3>
                <p style={{ margin: '0', color: '#999', fontSize: '0.9rem' }}>
                  {pedidoEncontrado.itens.length} {pedidoEncontrado.itens.length === 1 ? 'item' : 'itens'} • {formatarData(pedidoEncontrado.data)}
                </p>
              </div>
              <div style={{
                background: getStatusColor(pedidoEncontrado.rastreio.status) + '20',
        ...
        {pedidos.length > 0 && !pedidoEncontrado && (
          <div>
            ...
            {pedidos.map(function(pedido) {
              return (
                <div key={pedido.id} onClick={function() { selecionarPedido(pedido); }} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                    <div>
                      <h3 style={{ margin: '0 0 5px 0', color: '#333', fontSize: '1rem' }}>Pedido #{pedido.numero}</h3>
                      <p style={{ margin: '0', color: '#999', fontSize: '0.85rem' }}>
                        {pedido.itens.length} itens • R$ {pedido.total.toFixed(2)} • {formatarData(pedido.data)}
                      </p>
                    </div>
                    <div style={{
                      background: getStatusColor(pedido.rastreio.status) + '20',
                      color: getStatusColor(pedido.rastreio.status),
                      padding: '6px 12px', borderRadius: '15px',
                      fontWeight: 'bold', fontSize: '0.8rem'
                    }}>
                      {getStatusIcon(pedido.rastreio.status)} {pedido.rastreio.status}
                    </div>
                  </div>
                </div>
              );
